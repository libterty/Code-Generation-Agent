...
export class GitIntegrationServiceImpl {
  ...
  public async commitToGit(dto: RequestCommitGitDto) {
    const { repositoryUrl, branch, generatedCode, outputPath, requirementText, requirementAnalysis } = dto;
    ...
    const repoTempDir = fs.mkdtempSync(path.join(os.tmpdir(), `${this.config.templatePath}-${repoName}`));
    ...
    try {
      ...
      this.logger.log(`Cloning repository ${repositoryUrl} to ${repoTempDir}`);
      await git.clone(repositoryUrl, repoTempDir);
